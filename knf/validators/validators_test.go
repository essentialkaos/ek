...
func (s *ValidatorSuite) TestBasicValidators(c *check.C) {
	...
	c.Assert(errs[4].Error(), check.Equals, "Validator validators.Greater doesn't support input with type <string> for checking integer:test1 property")
...
[test]
  ...
  boolean: false`)
	...
	c.Assert(SizeGreater(cfg, "test:size", false).Error(), check.Equals, "Validator validators.SizeGreater doesn't support input with type <bool> for checking test:size property")
	c.Assert(SizeGreater(cfg, "test:size", "ABCD").Error(), check.Equals, `Invalid validators.SizeGreater validator value "ABCD"`)
	c.Assert(SizeGreater(cfg, "test:size", uint64(1*1024*1024)), check.IsNil)
	c.Assert(SizeGreater(cfg, "test:size", "1mb"), check.IsNil)
	c.Assert(SizeGreater(cfg, "test:empty", uint64(1*1024*1024)), check.IsNil)
	...
	c.Assert(SizeLess(cfg, "test:size", false).Error(), check.Equals, "Validator validators.SizeLess doesn't support input with type <bool> for checking test:size property")
	c.Assert(SizeLess(cfg, "test:size", "ABCD").Error(), check.Equals, `Invalid validators.SizeLess validator value "ABCD"`)
	c.Assert(SizeLess(cfg, "test:size", uint64(10*1024*1024)), check.IsNil)
	c.Assert(SizeLess(cfg, "test:size", "10mb"), check.IsNil)
	c.Assert(SizeLess(cfg, "test:empty", uint64(10*1024*1024)), check.IsNil)

	c.Assert(DurShorter(cfg, "test:dur", time.Minute).Error(), check.Equals, "Property test:dur can't be greater than 1m0s")
	c.Assert(DurShorter(cfg, "test:dur", 1234).Error(), check.Equals, "Validator validators.DurShorter doesn't support input with type <int> for checking test:dur property")
	...
	c.Assert(DurLonger(cfg, "test:dur", 1234).Error(), check.Equals, "Validator validators.DurLonger doesn't support input with type <int> for checking test:dur property")
	...
	c.Assert(InRange(cfg, "test:integer", false).Error(), check.Equals, "Validator validators.InRange doesn't support input with type <bool> for checking test:integer property")
	c.Assert(InRange(cfg, "test:integer", Range{true, 100.0}).Error(), check.Equals, "Validator validators.InRange doesn't support type <bool> for 'Range.From' value")
	c.Assert(InRange(cfg, "test:integer", Range{1.0, true}).Error(), check.Equals, "Validator validators.InRange doesn't support type <bool> for 'Range.To' value")
	c.Assert(InRange(cfg, "test:empty", Range{1.0, 100.0}), check.IsNil)
	...
	c.Assert(SetToAny(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(SetToAnyIgnoreCase(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(Less(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(Greater(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(NotEquals(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(HasPrefix(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(HasSuffix(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(LenShorter(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(LenLonger(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")
	c.Assert(LenEquals(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator validators..* doesn't support input with type <float32> for checking test:string property")

	c.Assert(HasPrefix(cfg, "test:string", ""), check.ErrorMatches, "Validator validators..* requires non-empty input for checking test:string property")
	c.Assert(HasSuffix(cfg, "test:string", ""), check.ErrorMatches, "Validator validators..* requires non-empty input for checking test:string property")
}
...
`)
	...
	c.Assert(TypeBool(cfg, "boolean:test8", nil).Error(), check.Equals, "Boolean property boolean:test8 contains unsupported value (disabled)")
	...
	c.Assert(TypeNum(cfg, "float:test3", nil).Error(), check.Equals, "Numeric property float:test3 contains unsupported value (0.6)")
	...
	c.Assert(TypeFloat(cfg, "float:test5", nil).Error(), check.Equals, "Float property float:test5 contains unsupported value (ABCD)")

	c.Assert(TypeSize(cfg, "size:test1", nil), check.IsNil)
	c.Assert(TypeSize(cfg, "size:test2", nil), check.IsNil)
	c.Assert(TypeSize(cfg, "size:test3", nil).Error(), check.Equals, "Size property size:test3 contains unsupported value (12.33km)")
	...
	c.Assert(TypeDur(cfg, "dur:test7", nil).Error(), check.Equals, "Time duration property dur:test7 contains unsupported value (12.4h)")
	c.Assert(TypeDur(cfg, "dur:test8", nil).Error(), check.Equals, "Time duration property dur:test8 contains unsupported value (1y)")
}
