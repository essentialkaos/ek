...
func (s *ValidatorSuite) TestBasicValidators(c *check.C) {
	...
	c.Assert(errs[4].Error(), check.Equals, "Validator Greater doesn't support input with type <string> for checking integer:test1 property")
...
[test]
  ...
  boolean: false`)
	...
	c.Assert(SizeGreater(cfg, "test:size", false).Error(), check.Equals, "Validator SizeGreater doesn't support input with type <bool> for checking test:size property")
	c.Assert(SizeGreater(cfg, "test:size", "ABCD").Error(), check.Equals, `Invalid SizeGreater validator value "ABCD"`)
	...
	c.Assert(SizeLess(cfg, "test:size", false).Error(), check.Equals, "Validator SizeLess doesn't support input with type <bool> for checking test:size property")
	c.Assert(SizeLess(cfg, "test:size", "ABCD").Error(), check.Equals, `Invalid SizeLess validator value "ABCD"`)
	...
	c.Assert(DurShorter(cfg, "test:dur", 1234).Error(), check.Equals, "Validator DurShorter doesn't support input with type <int> for checking test:dur property")
	...
	c.Assert(DurLonger(cfg, "test:dur", 1234).Error(), check.Equals, "Validator DurLonger doesn't support input with type <int> for checking test:dur property")
	...
	c.Assert(InRange(cfg, "test:integer", false).Error(), check.Equals, "Validator InRange doesn't support input with type <bool> for checking test:integer property")
	c.Assert(InRange(cfg, "test:integer", Range{true, 100.0}).Error(), check.Equals, "Validator InRange doesn't support type <bool> for 'Range.From' value")
	c.Assert(InRange(cfg, "test:integer", Range{1.0, true}).Error(), check.Equals, "Validator InRange doesn't support type <bool> for 'Range.To' value")
	...
	c.Assert(SetToAny(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(SetToAnyIgnoreCase(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(Less(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(Greater(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(NotEquals(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(HasPrefix(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(HasSuffix(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(LenShorter(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(LenLonger(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")
	c.Assert(LenEquals(cfg, "test:string", float32(1.1)), check.ErrorMatches, "Validator .* doesn't support input with type <float32> for checking test:string property")

	c.Assert(HasPrefix(cfg, "test:string", ""), check.ErrorMatches, "Validator .* requires non-empty input for checking test:string property")
	c.Assert(HasSuffix(cfg, "test:string", ""), check.ErrorMatches, "Validator .* requires non-empty input for checking test:string property")
}
