...
func (s *OptUtilSuite) TestAdd(c *C) {
	...
	c.Assert(opts.Add("tt:test5", &V{Alias: false}), ErrorMatches, `Option "--test5" contains unsupported list format of aliases`)
}
...
func (s *OptUtilSuite) TestGetters(c *C) {
	argline := "file.mp3 -SAT -s STRING -i 320 -b -f 1.098765 -S2 100 -f1 5 -f2 1 -ms ABC --merg-string DEF -mi 6 --merg-int 6 -f3 12 -mf 10.1 -mf 10.1 -i1 5 -- --bool"
	...
	c.Assert(args.Get(2).String(), Equals, "--")
	c.Assert(args.Get(3).String(), Equals, "--bool")
...
func (s *OptUtilSuite) TestArgumentsConversion(c *C) {
	...
	c.Assert(a.Get(1).Is(uint(6)), Equals, true)
	...
	c.Assert(vu, Equals, uint(0))
	vu, err = a.Get(1).Uint()
	c.Assert(err, IsNil)
	c.Assert(vu, Equals, uint(6))

	vu64, err := a.Get(0).Uint64()
	c.Assert(err, NotNil)
	c.Assert(err, ErrorMatches, `strconv.ParseUint: parsing "test": invalid syntax`)
	c.Assert(vu64, Equals, uint64(0))
	vu64, err = a.Get(1).Uint64()
	c.Assert(err, IsNil)
	c.Assert(vu64, Equals, uint64(6))
